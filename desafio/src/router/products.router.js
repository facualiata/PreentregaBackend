...
router.post("/", async (req, res) => {
    ...
    if (!title || !description || !code || !price || !stock || !category) {
      ...
      try {
        const response = await prodManager.addProduct(product);
        res.json({
  ...
  router.put("/:pid", async (req, res) => {
    ...
    const productTemp = {};
    let product = await prodManager.getProductById(parseInt(pid));
    ...
    if (product) {
      ...
      let result = await prodManager.updateProductById(
        parseInt(pid),
        productTemp
      );
  ...
  router.delete("/:pid", async (req, res) => {
    ...
    if (!product) {
      ...
      let result = await prodManager.deleteProductById(parseInt(pid));
